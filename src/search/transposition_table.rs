...
pub struct TransTable {
    ...
    pub tt: HashMap<Board, TransTableEntry>,
}
...
fn need_to_evict(table: &HashMap<Board, TransTableEntry>) -> bool {
    // Simple function, exists for readability
    table.len() > MAX_ENTRIES
}

fn evict(table: &mut HashMap<Board, TransTableEntry>) {
    ...
    let mut eviction_list: Vec<Board> = vec![];
...
fn least_depth_entries(table: &HashMap<Board, TransTableEntry>, eviction_list: &mut Vec<Board>) {
